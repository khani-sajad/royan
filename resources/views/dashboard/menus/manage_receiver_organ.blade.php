@php
  $open = in_array( $dashboard,['base_management','base_offer','z2','z3','z4','z5','z6'] )
@endphp
<hr class="p-0 m-0">
...
<div id="manageReceiverOrgan" class="drop @if (!$open) {{'hidden'}} @endif">
  <a href="{{url(userable_type().'/base_management')}}" @if ($dashboard=='base_management') {{'class=active'}} @endif>
    <i class="fa fa-pencil pl-1"></i>
    <span> ویرایش اطلاعات پایه </span>
  </a>
  <a href="{{url(userable_type().'/base_offer')}}" @if ($dashboard=='base_offer') {{'class=active'}} @endif>
    <i class="fa fa-percent pl-1"></i>
    <span> ویرایش آفر پایه </span>
  </a>
