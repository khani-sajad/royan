...
<div id="manageTransactions" class="drop @if (!$open) {{'hidden'}} @endif">
  <a href="{{url(userable_type().'/new_transaction')}}" @if ($dashboard=='new_transaction') {{'class=active'}} @endif>
    <i class="fa fa-plus pl-1"></i>
    <span> ثبت تراکنش آیکیو کارت </span>
  </a>
  <a href="{{url(userable_type().'/new_barg_transaction')}}" @if ($dashboard=='new_barg_transaction') {{'class=active'}} @endif>
    <i class="fa fa-plus pl-1"></i>
  ...
  <a href="{{url(userable_type().'/transactions_list')}}" @if ($dashboard=='transactions_list') {{'class=active'}} @endif>
    <i class="fa fa-eye pl-1"></i>
    <span> مشاهده تراکنش&zwnj;های آیکیو کارت </span>
  </a>
  <a href="{{url(userable_type().'/barg_transactions_list')}}" @if ($dashboard=='barg_transactions_list') {{'class=active'}} @endif>
    <i class="fa fa-eye pl-1"></i>
